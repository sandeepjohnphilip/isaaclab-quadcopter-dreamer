...
def watch():
    ...
    if cfg.obstacle_enabled:
        if args.random_obstacle:
            # Sample random obstacle inside corridor
            margin_x = 2.0
    ...
    if (not args.headless) and args.view_delay > 0.0:
        print(f"\nWaiting {args.view_delay:.1f}s to adjust camera...\n")
    ...
    if reached_goal_view:
        print(
            ...
            f"(threshold {success_pos_threshold:.2f}m, goal {goal_x:.2f}m)"
        )
    else:
        ...
        if cb.get("wall", 0) > 0:
            ...
            crash_type = "TIMEOUT"
        print(f"FAILED: {crash_type} at x={max_x:.2f}m (goal {goal_x:.2f}m)")
    print(f"\nSummary:")
